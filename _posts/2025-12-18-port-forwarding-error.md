...
확인해보니 나는 공유기를 사용한 환경이여서 내부망으로 접속 성공한 것이였고, 친구는 외부 ip니깐 접속이 안되었던 것이다.  
...
문제는 내 와이파이 공유기 구성의 문제였다.  
 ...
 ## 2. 해결 시도
    ...
    3. 도메인을 부여해보자!
        ...
        - 여러번 검색을 해본 결과, NAT Loopback 이라는 현상때문일 수도 있다는 해결책을 찾았다.  
...
진짜 스트레스 받는 일이였다. 나는 분명히 세팅을 완료했는데 제대로 작동하지 않는다니...  
그러나 해결책을 찾고 나니 조금 더 네트워크 환경에 대해 접근성을 가지기도 했고,  
하드웨어 세팅도 중요하다는 교훈을 얻었다.  
그래도 무사히 핸드폰으로 작동되는걸 확인하니 매우 뿌듯하다.